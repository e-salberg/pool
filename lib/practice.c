...
void InitPracticeScene() {
  ...
  for (int col = 0; col < COLS; col++) {
    float baseHeight = ((float)GetScreenHeight() / 2) - (col * BALL_RADIUS);
    for (int i = 0; i <= col; i++) {
      int ball = startingOrder[idx];
      physics[ball].position.x =
          ((float)GetScreenWidth() * .6f) + (col * 1.75 * BALL_RADIUS);
      physics[ball].position.y = baseHeight + (i * 2 * BALL_RADIUS);
      idx++;
    }
  }
  physics[0].position.x = (float)GetScreenWidth() / 4;
  physics[0].position.y = (float)GetScreenHeight() / 2;
}
